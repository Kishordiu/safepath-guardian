...
export function MapCard({
  ...
  return (
    <div className={cn('rounded-2xl border bg-card overflow-hidden shadow-safe-md', height, className)}>
      <MapContainer
      ...
      >
        ...
        {geofences.map((g, i) => (
          <Circle
            ...
            pathOptions={{
              color: g.type === 'home' ? 'hsl(220,70%,45%)' : g.type === 'school' ? 'hsl(152,60%,42%)' : 'hsl(38,92%,50%)',
              fillOpacity: 0.08,
              weight: 2,
            }}
          >
        ...
        {stopPoints.map((sp, i) => (
          <Circle
            ...
            radius={30}
            pathOptions={{
              color: sp.isExpected ? 'hsl(220,70%,45%)' : 'hsl(25,95%,53%)',
              fillOpacity: 0.3,
              weight: 2,
            }}
          >
            <Popup>
              <div className="text-sm">
                <p className="font-medium">{sp.label || 'Stop Point'}</p>
                <p className="text-muted-foreground">{sp.durationMins} min</p>
              </div>
            </Popup>
          </Circle>
        ))}

        {childPosition && <Marker position={[childPosition.lat, childPosition.lng]} icon={childIcon}>
          <Popup><span className="font-medium text-sm">Current Location</span></Popup>
        </Marker>}
      </MapContainer>
    </div>
  );
